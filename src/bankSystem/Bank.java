...
public class Bank {
	ArrayList<Customer> customers = new ArrayList<Customer>(); // Any classes in
																// the package
																// can access
																// this without
																// protection
	ArrayList<Employee> employees = new ArrayList<Employee>(); // Any classes in
																// the package
																// can access
																// this without
																// protection
	ArrayList<IAccount> all_accounts; // Any classes in the package can access
	...
	synchronized void addAccount(IAccount acc) {
    	if (all_accounts == null) {
            synchronized(this) {
                if (all_accounts == null) {
                    all_accounts = new ArrayList<IAccount>();();
                }
            }
        }
